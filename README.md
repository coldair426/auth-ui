...
- **표준 카드 시스템 (400px Unified Width)**: 모든 주요 인증 카드는 데스크탑 안정감을 위해 **400px** 너비로 통일되어 있으며, 모바일에서는 유연한 여백 시스템과 함께 자연스러운 스크롤링을 지원합니다.
- **모바일 최적화 (Mobile-First)**: iPhone SE 등 소형 기기에서도 로고 크기, 여백, 폰트가 동적으로 조절되어 최상의 시네마틱 경험을 유지합니다. 중첩 스크롤 없는 전체 뷰포트 스크롤 시스템을 채택했습니다.
- **시네마틱 질감 통일 (Unified Cinematic Texture)**: 필름 그레인 오버레이를 모바일/데스크탑 모두에 적용합니다. 기기별 opacity를 달리하여 Galaxy 등 모바일 기기 호환성과 디자인 완성도를 동시에 확보합니다. 배경 Orb 및 Shimmer 효과도 절제된 수치로 관리하여 질감이 UI를 해치지 않도록 합니다.
