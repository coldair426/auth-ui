...
- **Cinematic Texture (Unified)**: 모바일/데스크탑 공통으로 SVG 필름 그레인(Grain) 오버레이를 적용하여 아날로그적인 질감을 부여합니다. `mix-blend-overlay` 사용. 기기별 opacity: 모바일/데스크탑 공통 `0.05`(라이트) / `0.07`(다크). 과도한 효과로 디자인이 손상되지 않도록 의도적으로 낮은 수치를 유지합니다.
